...
const isRectangle = mod.object({
  ...
  isUnregisterMenuEvent = mod.object({ key: mod.string() }),
  ...
  isOpenMenuEvent = mod.object({ key: mod.string() }),
  ...
  isCreateDynamicAreaEvent = mod.object({
    name: mod.string(),
    x: mod.number(),
    y: mod.number(),
    width: mod.number(),
...
mod.object({
  ...
  "api/domain/verify": src.extendApi(mod.optional(mod.string()), {
    description:
      "Means the api can validate if a domain is a legitimate domain. Needed if you do OAuth login AND your WorkAdventure install supports multiple domains.",
    example: "v1",
  }),
});
const CompanionBehavior = mod.union([
    mod.literal("cat"),
    mod.literal("dog"),
    mod.literal("red_panda"),
  ]),
  CompanionTexture = mod.object({
...
const isMetaTagFavicon = mod.object({
...
mod.object({
  mapUrl: src.extendApi(mod.string().optional(), {
    description: "The full URL to the JSON map file",
    example: "https://myuser.github.io/myrepo/map.json",
  }),
  wamUrl: src.extendApi(mod.string().url().optional(), {
  ...
  mucRooms: src.extendApi(MucRoomDefinition.array().nullable(), {
  ...
  errorSceneLogo: src.extendApi(mod.string().nullable().optional(), {
...
const isXmppSettingsMessageEvent = mod.object({
  ...
  isBannerEvent = mod.object({
    ...
    timeToClose: mod.number().optional().default(12e4),
...
mod.union([
  ...
  mod.object({ type: mod.literal("openMenu"), data: isOpenMenuEvent }),
  ...
  mod.object({ type: mod.literal("modifyArea"), data: isDynamicAreaEvent }),
...
class WorkadventureNavigationCommands extends IframeApiContribution {
  ...
  goToLogin() {
    return queryWorkadventure({ type: "goToLogin", data: void 0 });
  }
}
...
function unsafeStringify(r, e = 0) {
  return (
    byteToHex[r[e + 0]] +
    byteToHex[r[e + 1]] +
    byteToHex[r[e + 2]] +
...
const randomUUID =
  ...
  native = { randomUUID };
function v4(r, e, t) {
  if (native.randomUUID && !e && !r) return native.randomUUID();
  r = r || {};
  const o = r.random || (r.rng || rng)();
  if (((o[6] = (o[6] & 15) | 64), (o[8] = (o[8] & 63) | 128), e)) {
    t = t || 0;
    for (let n = 0; n < 16; ++n) e[t + n] = o[n];
    return e;
  }
...
class Menu {
  ...
  remove() {
    sendToWorkadventure({ type: "unregisterMenu", data: { key: this.key } });
  }
  open() {
    sendToWorkadventure({ type: "openMenu", data: { key: this.key } });
  }
}
...
class WorkAdventureUiCommands extends IframeApiContribution {
  ...
  registerMenuCommand(t, o) {
    ...
    const n = {
        ...o,
        allowApi: o.allowApi === void 0 ? o.iframe !== void 0 : o.allowApi,
        key: (s = o.key) != null ? s : v4(),
      },
      i = new Menu(n.key);
    if (n.iframe !== void 0)
      sendToWorkadventure({
        ...
        data: {
          ...
          iframe: n.iframe,
          key: n.key,
          options: { allowApi: n.allowApi },
        },
      });
    else if (n.callback !== void 0)
      menuCallbacks.set(n.key, n.callback),
        sendToWorkadventure({
          type: "registerMenu",
          data: { name: t, key: n.key, options: { allowApi: n.allowApi } },
        });
    else
      throw new Error(
        "When adding a menu with WA.ui.registerMenuCommand, you must pass either an iframe or a callback"
      );
    return menus.set(n.key, i), i;
  }
  getMenuCommand(t) {
    return Promise.resolve(new Menu(t));
  }
...
class WorkadventureStateCommands extends AbstractWorkadventureStateCommands {
  ...
  saveVariable(t, o) {
    if (this.variables.get(t) === o) return new Promise((i) => i());
    this.variables.set(t, o);
    const n = this.variableSubscribers.get(t);
    return (
      n && n.next(o),
